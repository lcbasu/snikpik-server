                ...
                UserAggregateActivityType.NEW_POST_CREATED -> {
                    ...
                    when (userActivity.userActivityType) {
                        UserActivityType.POST_CREATED -> {
                            ...
                            body = "Checkout the new story post by " + ((if (CommonUtils.isValidString(byUser.handle)) byUser.handle else byUser.fullName))
                        }
                        UserActivityType.WALL_CREATED -> {
                            title = "You have a new post on forum"
                            body = "Checkout the new question post by ${((if (CommonUtils.isValidString(byUser.handle)) byUser.handle else byUser.fullName))}, and comment to show your response."
                        }
                        else -> error("Invalid userActivityType: ${userActivity.userActivityType} for userActivityId: ${userActivity.userActivityId}")
                    }

                    landingUrl = "${CommonUtils.UNBOX_ROOT_URL}/posts/${dataValue3}"
                }
                UserAggregateActivityType.LIKED -> {
                    ...
                    when (userActivity.userActivityType) {
                        UserActivityType.POST_LIKED -> {
                            ...
                            body = "${((if (CommonUtils.isValidString(byUser.handle)) byUser.handle else byUser.fullName))} liked your post."
                            mediaURL = getMediaUrlForNotification(userActivity.postMediaDetails)
                        }
                        UserActivityType.WALL_LIKED -> {
                            title = "Your forum post has a new like"
                            body = "${((if (CommonUtils.isValidString(byUser.handle)) byUser.handle else byUser.fullName))} liked your forum post."
                            mediaURL = getMediaUrlForNotification(userActivity.postMediaDetails)
                        }
                        UserActivityType.POST_COMMENT_LIKED,
                        UserActivityType.WALL_COMMENT_LIKED-> {
                            title = "Your comment has a new like"
                            body = "${((if (CommonUtils.isValidString(byUser.handle)) byUser.handle else byUser.fullName))} liked your comment: ${userActivity.commentText}."
                        ...
                        UserActivityType.WALL_COMMENT_REPLY_LIKED -> {
                            ...
                            body = "${((if (CommonUtils.isValidString(byUser.handle)) byUser.handle else byUser.fullName))} liked your reply: ${userActivity.replyText}."
                ...
                UserAggregateActivityType.SAVED -> {
                    ...
                    when (userActivity.userActivityType) {
                        UserActivityType.POST_SAVED -> {
                            ...
                            body = "${((if (CommonUtils.isValidString(byUser.handle)) byUser.handle else byUser.fullName))} saved your story post."
                        }
                        UserActivityType.WALL_SAVED -> {
                            title = "Your forum post was saved"
                            body = "${((if (CommonUtils.isValidString(byUser.handle)) byUser.handle else byUser.fullName))} saved your forum post."
                        }
                        else -> error("Invalid userActivityType: ${userActivity.userActivityType} for userActivityId: ${userActivity.userActivityId}")
                    }
                    landingUrl = "${CommonUtils.UNBOX_ROOT_URL}/posts/${dataValue3}"
                ...
                UserAggregateActivityType.SHARED -> {
                    when (userActivity.userActivityType) {
                        UserActivityType.POST_SHARED -> {
                            ...
                            body = "${((if (CommonUtils.isValidString(byUser.handle)) byUser.handle else byUser.fullName))} shared your story post."
                            mediaURL = getMediaUrlForNotification(userActivity.postMediaDetails)
                            dataValue3 = userActivity.postId ?: error("Missing post id for activityId: ${userActivity.userActivityId}")
                        }
                        UserActivityType.WALL_SHARED -> {
                            title = "Your forum post was shared"
                            body = "${((if (CommonUtils.isValidString(byUser.handle)) byUser.handle else byUser.fullName))} shared your forum post."
                            mediaURL = getMediaUrlForNotification(userActivity.postMediaDetails)
                            dataValue3 = userActivity.postId ?: error("Missing post id for activityId: ${userActivity.userActivityId}")
                        }
                        UserActivityType.USER_PROFILE_SHARED -> {
                            title = "Your profile was shared"
                            body = "${((if (CommonUtils.isValidString(byUser.handle)) byUser.handle else byUser.fullName))} shared your profile."
                ...
                UserAggregateActivityType.COMMENTED -> {
                    ...
                    when (userActivity.userActivityType) {
                        UserActivityType.COMMENTED_ON_POST -> {
                            ...
                            body = "${((if (CommonUtils.isValidString(byUser.handle)) byUser.handle else byUser.fullName))} commented on your story post: ${userActivity.commentText}"
                        }
                        UserActivityType.COMMENTED_ON_WALL -> {
                            title = "Comment on your forum post"
                            body = "${((if (CommonUtils.isValidString(byUser.handle)) byUser.handle else byUser.fullName))} commented on your forum post: ${userActivity.commentText}"
                        }
                        else -> error("Invalid userActivityType: ${userActivity.userActivityType} for userActivityId: ${userActivity.userActivityId}")
                    }
                    landingUrl = "${CommonUtils.UNBOX_ROOT_URL}/posts/${dataValue3}"
                ...
                UserAggregateActivityType.REPLIED -> {
                    ...
                    when (userActivity.userActivityType) {
                        UserActivityType.REPLIED_TO_POST_COMMENT -> {
                            ...
                            body = "${(if (CommonUtils.isValidString(byUser.handle)) byUser.handle else byUser.fullName)} replied on your comment in story post: ${userActivity.replyText}"
                        }
                        UserActivityType.REPLIED_TO_WALL_COMMENT -> {
                            title = "Your comment has a new reply"
                            body = "${(if (CommonUtils.isValidString(byUser.handle)) byUser.handle else byUser.fullName)} replied on your comment in forum post: ${userActivity.replyText}"
                        }
                        else -> error("Invalid userActivityType: ${userActivity.userActivityType} for userActivityId: ${userActivity.userActivityId}")
                    }
                    landingUrl = "${CommonUtils.UNBOX_ROOT_URL}/posts/${dataValue3}"
                ...
                UserAggregateActivityType.FOLLOWED -> {
                    ...
                    when (userActivity.userActivityType) {
                        UserActivityType.USER_FOLLOWED -> {
                            ...
                            body = "${(if (CommonUtils.isValidString(byUser.handle)) byUser.handle else byUser.fullName)} followed you."
                        }
                        else -> error("Invalid userActivityType: ${userActivity.userActivityType} for userActivityId: ${userActivity.userActivityId}")
                    }
                    landingUrl = "${CommonUtils.UNBOX_ROOT_URL}/users/${dataValue3}"
                }
                UserAggregateActivityType.CLICKED_CONNECT -> {
                    ...
                    when (userActivity.userActivityType) {
                        UserActivityType.USER_CLICKED_CONNECT -> {
                            ...
                            body = "${(if (CommonUtils.isValidString(byUser.handle)) byUser.handle else byUser.fullName)} tried reaching out to you through your contact details."
                        }
                        else -> error("Invalid userActivityType: ${userActivity.userActivityType} for userActivityId: ${userActivity.userActivityId}")
                    }
                    landingUrl = "${CommonUtils.UNBOX_ROOT_URL}/users/${dataValue3}"
                ...
                UserAggregateActivityType.MESSAGE_SENT_OR_RECEIVED -> {
                    ...
                    when (userActivity.userActivityType) {
                        UserActivityType.USER_SENT_CHAT_MESSAGE -> {
                            ...
                            body = "${(if (CommonUtils.isValidString(byUser.handle)) byUser.handle else byUser.fullName)} sent you a chat message: ${userActivity.chatText}"
                        }
                        else -> error("Invalid userActivityType: ${userActivity.userActivityType} for userActivityId: ${userActivity.userActivityId}")
                    }
                }
